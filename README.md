...
| Role | texture0 | texture1 | texture2 | texture3 | Writes to |
|------|----------|----------|----------|----------|-----------|
| `:state` (1st shader) | original image | audio | prev frame (ping-pong) | waveform PCM | ping-pong buf |
| `:display` (auto-expanded `_display`) | original image | audio | current state buf | waveform PCM | chain buf |
| `:post` (2nd+ shader) | previous pass output | audio | original image | waveform PCM | chain buf |
