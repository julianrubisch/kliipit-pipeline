...
void main() {
    ...
    // --- Tape wobble: horizontal displacement that increases with bass ---
    float wobbleAmount = 0.003 + bass * 0.015;
    float wobble = sin(uv.y * 30.0 + u_time * 3.0) * wobbleAmount;
    wobble += sin(uv.y * 120.0 + u_time * 7.0) * wobbleAmount * 0.3;
    uv.x += wobble;
